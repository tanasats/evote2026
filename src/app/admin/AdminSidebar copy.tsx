...
import {
  ...
  Calendar, BarChart3, ShieldAlert, ChevronRight, Menu, X, PanelLeftOpen,
  Activity,
  Vote
} from 'lucide-react';

export default function xxAdminSidebar() {
  ...
  const menuGroups = [
    {
      ...
      roleRequired: 'ADMIN', // ADMIN เห็นได้ (รวมถึง SUPER_ADMIN ด้วยตาม Logic ด้านล่าง)
      items: [
    ...
    },
    {
      ...
      roleRequired: 'SUPER_ADMIN', // กำหนดว่าต้องเป็น Super Admin เท่านั้น
      items: [
  ...
  // ฟังก์ชันช่วยปิดเมนูเมื่อกดลิงก์ (สำหรับ Mobile)
  const closeSidebar = () => setIsOpen(false);

  return (
    <>
      {/* --- Mobile Header: แสดงเฉพาะบนหน้าจอเล็ก --- */}
      ...
      {/* --- Overlay Backdrop: คลิกพื้นหลังเพื่อปิดเมนู (Mobile เท่านั้น) --- */}
      {isOpen && (
        <div
      ...
      {/* --- Sidebar Container --- */}
      {/* <aside className={`
        fixed inset-y-0 left-0 z-[80] w-72 bg-slate-900 text-slate-300 p-6 
      ...
      `}>
        ...
        <div className="flex items-center justify-between py-3 ">
          <div className='block lg:hidden'>
            ...
            {/* <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mt-1">Super Admin Access</p> */}
          </div>
          {/* ปุ่มปิดบนมือถือ */}
        ...
        {/* Navigation Menu */}
        <nav className="space-y-4 overflow-y-auto max-h-[calc(100vh-160px)] custom-sidebar-scroll">
        ...
        {/* Footer ของ Sidebar (Logout หรือ Info) */}
        <div className="absolute bottom-6 left-6 right-6 pt-6 border-t border-slate-800">
          <p className="text-[10px] text-slate-600 font-medium text-center italic">© 2026 E-VOTE System v1.0</p>
        </div>
      </aside>
    </>
  );
